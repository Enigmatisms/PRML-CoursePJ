...
class PatchEmbeddings(nn.Module):
    ...
    def __init__(self, ksize = 3, initial_ksize = 17, out_channels = 128) -> None:
        super().__init__()
        self.convs = nn.Sequential(
            *makeConv1D(4,                  out_channels << 2,  initial_ksize,    norm = nn.BatchNorm1d(out_channels << 2)),
            *makeConv1D(out_channels << 2,  out_channels,       ksize,            norm = nn.BatchNorm1d(out_channels),      max_pool = 2),
            *makeConv1D(out_channels,       out_channels,       ksize,            norm = nn.BatchNorm1d(out_channels),      max_pool = 2),
        )
...
class SeqPredictor(nn.Module):
    ...
    def __init__(self, args, emb_dim = 256) -> None:
        super().__init__()
        self.emb_dim = emb_dim
        linear_dim = emb_dim << 1
        
        self.patch_embed = PatchEmbeddings(5, 17, emb_dim)
        self.avg_pool = nn.AdaptiveAvgPool1d(1)
        self.emb_drop = nn.Dropout(args.emb_dropout)
        self.drop_path = DropPath(args.path_dropout)
        
        self.input_layers = nn.Sequential(
            *makeConv1D(emb_dim,        emb_dim,        3, norm = nn.BatchNorm1d(emb_dim),      max_pool = 2),
        ...
        self.out_layers = nn.Sequential(
            *makeMLP(linear_dim, linear_dim, args.class_dropout),
            *makeMLP(linear_dim, linear_dim, args.class_dropout),
